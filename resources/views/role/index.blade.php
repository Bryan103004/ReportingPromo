...
@section('content')
    ...
    <div class="mb-5 flex items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-md font-medium">Daftar role yang tersedia.</p>
        </div>
        ...
        {{-- Form Paginate Atas --}}
        <form method="GET" action="{{ route('role.index') }}" id="paginateForm" class="paginateForm inline-block mb-4">
        ...
        <form method="GET" action="{{ route('role.index') }}" id="paginateFormBottom" class="paginateForm inline-block mt-2 mb-2">
            @foreach(request()->except('number') as $key => $val)
                @if(is_array($val))
                    @foreach($val as $v)
