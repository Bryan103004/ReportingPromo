...
<div class="mx-auto max-w-7xl px-4 py-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="bg-blue-600 px-6 py-4 flex justify-between items-center">
            <h5 class="text-lg font-bold text-white m-0">Edit Data Jsm</h5>
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('jsm.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm">
                    Kembali
                </a>
            </div>
        </div>
        ...
        <form action="{{route('jsm.update', $jsm->id)}}" method="POST" class="p-6" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="relative flex-1">
                            <select name="supplier_code" id="choices-supplier" 
                                ...
                                @foreach ($supplierRafaksi as $supplier)
                                    <option value="{{ $supplier->kode_supplier }}" {{ old('supplier_code', $jsm->supplier_code) == $supplier->kode_supplier ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $jsm->supplier_name) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Otomatis Terisi Saat Anda Memilih Kode Supplier" readonly required>
                </div>    
                    ...
                    <div class="relative">
                        <select name="category_id" id="category_id" 
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $jsm->category_id) == $category->id ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="no_raf" id="no_raf" value="{{ old('no_raf', $jsm->no_raf) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Contoh: RAF/2026/001" required>
                </div>

                {{-- No. Shiji --}}
                <div>
                    <label for="no_shiji" class="block text-sm font-semibold text-gray-700 mb-1.5">No. Shiji</label>
                    <input type="text" name="no_shiji" id="no_shiji" value="{{ old('no_shiji', $jsm->no_shiji) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Nomor dokumen Shiji (opsional)">
                </div>

                {{-- No. Shiji Promotion --}}
                <div>
                    <label for="no_shiji_promotion" class="block text-sm font-semibold text-gray-700 mb-1.5">No. Shiji Promotion</label>
                    <input type="text" name="no_shiji_promotion" id="no_shiji_promotion" value="{{ old('no_shiji_promotion', $jsm->no_shiji_promotion) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Nomor dokumen Shiji Promotion (opsional)">
                </div>
            ...
            <div class="mb-8 mx-6">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="nominal" id="nominal" min="0" step="any" value="{{ old('nominal', $jsm->nominal) }}" class="w-full rounded-md border border-gray-300 pl-10 pr-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="0" required>
                </div>
            ...
            <div class="mb-8 mx-6">
                ...
                <textarea name="remarks" id="remarks" rows="3" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors @error('remarks') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror" placeholder="Masukkan catatan di sini...">{{ old('remarks', $jsm->remarks) }}</textarea>
            </div>

            <div class="mb-8 mx-6">
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Dokumen (PDF)</label>

                @if ($jsm->documents->isNotEmpty())
                    <ul class="mb-3 divide-y divide-gray-200 rounded-md border border-gray-200">
                        @foreach ($jsm->documents as $doc)
                            <li class="flex items-center justify-between gap-3 px-4 py-2 text-sm">
                                <a href="{{ route('jsm.documents.download', $doc->id) }}" class="text-blue-600 hover:underline truncate" target="_blank">
                                    {{ $doc->filename }}
                                </a>
                                <button type="button" onclick="submitDocumentDelete('{{ route('jsm.documents.destroy', $doc->id) }}')" class="text-red-500 hover:text-red-700 text-xs font-semibold shrink-0">Hapus</button>
                            </li>
                        @endforeach
                    </ul>
                @endif
            ...
            <div class="m-4 flex items-center justify-end gap-3 pt-5 border-t border-gray-100">
                <a href="{{ route('jsm.show_month', ['year' => $year, 'month' => $month, 'page' => request('page')]) }}" class="px-5 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Batal</a>
...
@php
    $itemRowsForJs = $jsm->items->map(function ($item) {
        return [
...
<script>
    ...
    window.selectedTokos = @json($jsm->tokos->pluck('id'));
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const initialRegionIds = @json($jsm->tokos->pluck('region_id')->unique()->values());
