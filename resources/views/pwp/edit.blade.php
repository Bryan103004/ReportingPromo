        ...
        <form action="{{route('pwp.update', $pwp->id)}}" method="POST" class="p-6" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="relative flex-1">
                            <select name="supplier_code" id="choices-supplier"
                                ...
                                @foreach ($supplierRafaksi as $supplier)
                                    <option value="{{ $supplier->kode_supplier }}" {{ old('supplier_code', $pwp->supplier_code) == $supplier->kode_supplier ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $pwp->supplier_name) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Otomatis Terisi Saat Anda Memilih Kode Supplier" readonly required>
                </div>
                    ...
                    <div class="relative">
                        <select name="category_id" id="category_id"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $pwp->category_id) == $category->id ? 'selected' : '' }} class="text-gray-900">
                ...
                {{-- Periode Awal --}}
                <div>
                    <label for="periode_awal" class="block text-sm font-semibold text-gray-700 mb-1.5">Periode Awal PWP<span class="text-red-500">*</span></label>
                    <input type="date" name="periode_awal" id="periode_awal" value="{{ old('periode_awal', $pwp->periode_awal ? date('Y-m-d', strtotime($pwp->periode_awal)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div>

                {{-- Periode Akhir --}}
                <div>
                    <label for="periode_akhir" class="block text-sm font-semibold text-gray-700 mb-1.5">Periode Akhir PWP<span class="text-red-500">*</span></label>
                    <input type="date" name="periode_akhir" id="periode_akhir" value="{{ old('periode_akhir', $pwp->periode_akhir ? date('Y-m-d', strtotime($pwp->periode_akhir)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="no_shiji_promotion" id="no_shiji_promotion" value="{{ old('no_shiji_promotion', $pwp->no_shiji_promotion) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Nomor dokumen Shiji Promotion (opsional)">
                </div>
                ...
                <div>
                    ...
                    <select name="status_email" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>                        <option value="aktif" {{ old('status_email', $pwp->status_email) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="tidak_aktif" {{ old('status_email', $pwp->status_email) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="periode_bulan" id="periode_bulan" value="{{ old('periode_bulan', $pwp->periode_bulan ? date('Y-m-d', strtotime($pwp->periode_bulan)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div>
                ...
                <input type="hidden" name="store" id="hidden_store_name" value="{{ old('store', $pwp->store) }}">
            ...
            <div class="m-4 flex items-center justify-end gap-3 pt-5 border-t border-gray-100">
                <a href="{{ route('pwp.show_month', ['year' => $year, 'month' => $month, 'page' => request('page')]) }}" class="px-5 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    Batal
                </a>
...
<script>
    ...
    function getNextNoRaf(){
        ...
        url.searchParams.set('category', 'PWP');
