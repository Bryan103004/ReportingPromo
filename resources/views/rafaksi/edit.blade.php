...
<div class="mx-auto max-w-7xl px-4 py-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="bg-blue-600 px-6 py-4 flex justify-between items-center">
            <h5 class="text-lg font-bold text-white m-0">Edit Data Rafaksi</h5>
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('rafaksi.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm">
                    Kembali
                </a>
            </div>
        </div>
        ...
        <div class="p-6 md:p-8">
            ...
            @if ($errors->any())
                <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-md">
                    <ul class="text-sm text-red-700 list-disc list-inside m-0">
        ...
        <form action="{{route('rafaksi.update', $rafaksi->id)}}" method="POST" class="p-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <select name="supplier_code" id="choices-supplier" 
                                ...
                                required>
                                <option value="" disabled class="text-gray-400">Pilih atau cari supplier...</option>
                                @foreach ($supplierRafaksi as $supplier)
                                    <option value="{{ $supplier->kode_supplier }}" {{ old('supplier_code', $rafaksi->supplier_code) == $supplier->kode_supplier ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $rafaksi->supplier_name) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Otomatis Terisi Saat Anda Memilih Kode Supplier" readonly required>
                </div>    
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="category_id" id="category_id" 
                            ...
                            required>
                            <option value="" disabled class="text-gray-400">-- Pilih Category --</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $rafaksi->category_id) == $category->id ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="date" name="periode_awal" id="periode_awal" value="{{ old('periode_awal', $rafaksi->periode_awal ? date('Y-m-d', strtotime($rafaksi->periode_awal)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="periode_akhir" id="periode_akhir" value="{{ old('periode_akhir', $rafaksi->periode_akhir ? date('Y-m-d', strtotime($rafaksi->periode_akhir)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div> 

                {{-- No. RAF --}}
                <div>
                    <label for="no_raf" class="block text-sm font-semibold text-gray-700 mb-1.5">No. RAF <span class="text-red-500">*</span></label>
                    <input type="text" name="no_raf" id="no_raf" value="{{ old('no_raf', $rafaksi->no_raf) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Contoh: RAF/2026/001" required>
                </div>
                ...
                <div>
                    ...
                    <select name="status_email" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>                        <option value="aktif" {{ old('status_email', $rafaksi->status_email) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="tidak_aktif" {{ old('status_email', $rafaksi->status_email) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="periode_bulan" id="periode_bulan" value="{{ old('periode_bulan', $rafaksi->periode_bulan ? date('Y-m-d', strtotime($rafaksi->periode_bulan)) : '') }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                </div>
                ...
                <input type="hidden" name="store" id="hidden_store_name" value="{{ old('store', $rafaksi->store) }}">
                <input type="hidden" name="raf_sequence" id="raf_sequence" value="{{ old('raf_sequence', $rafaksi->raf_sequence) }}">
            </div>
            ...
            <div class="mb-8 mx-6">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="nominal" id="nominal" min="0" step="any" value="{{ old('nominal', $rafaksi->nominal) }}" class="w-full rounded-md border border-gray-300 pl-10 pr-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="0" required>
                </div>
            ...
            <div class="mb-8 mx-6">
                ...
                <textarea name="remarks" id="remarks" rows="3" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors @error('remarks') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror" placeholder="Masukkan catatan di sini...">{{ old('remarks', $rafaksi->remarks) }}</textarea>
            </div> 

            <div class="mb-8 mx-6">
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Dokumen (PDF)</label>

                @if ($rafaksi->documents->isNotEmpty())
                    <ul class="mb-3 divide-y divide-gray-200 rounded-md border border-gray-200">
                        @foreach ($rafaksi->documents as $doc)
                            <li class="flex items-center justify-between gap-3 px-4 py-2 text-sm">
                                <a href="{{ route('rafaksi.documents.download', $doc->id) }}" class="text-blue-600 hover:underline truncate" target="_blank">
                                    {{ $doc->filename }}
                                </a>
                                <button type="button" onclick="submitDocumentDelete('{{ route('rafaksi.documents.destroy', $doc->id) }}')" class="text-red-500 hover:text-red-700 text-xs font-semibold shrink-0">Hapus</button>
                            </li>
                        @endforeach
                    </ul>
                @endif
            ...
            <div class="m-4 flex items-center justify-end gap-3 pt-5 border-t border-gray-100">
                <a href="{{ route('rafaksi.show_month', ['year' => $year, 'month' => $month, 'page' => request('page')]) }}" class="px-5 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    Batal
                </a>
...
@php
    $itemRowsForJs = $rafaksi->items->map(function ($item) {
        return [
...
<script>
    ...
    window.selectedTokos = @json($rafaksi->tokos->pluck('id'));
    ...
    function renderItemsTable(){
        ...
        window.itemRows.forEach((row, i) => {
            ...
            bodyHtml += `<td class="px-1 py-1 border text-center"><button type="button" onclick="removeItemRow(${i})" class="text-red-500 hover:text-red-700 font-bold" title="Hapus baris">&times;</button></td>`;
            bodyHtml += '</tr>';
        });
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const initialRegionIds = @json($rafaksi->tokos->pluck('region_id')->unique()->values());
