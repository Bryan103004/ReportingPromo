...
<body>
    <div class="email-container">
        ...
        <div class="content">
            <h2 style="font-size: 18px; color: #111827;">Peringatan Masa Berlaku Rafaksi</h2>
            <p>Halo <strong>{{ $user->name }}</strong>,</p>
            <p>Berikut adalah daftar Rafaksi yang telah memasuki periode pengurusan renewal berdasarkan setting reminder:</p>
            ...
            <table>
                ...
                <tbody>
                    @foreach ($rafaksis as $rafaksi)
                    <tr>
                        <!-- Kolom 1: Toko -->
                        <td>{{ $rafaksi->tokos->pluck('nama_toko')->implode(', ') ?: '-' }}</td>
                        
                        <!-- Kolom 2: Nomor RAF -->
                        <td style="font-family: monospace;">{{ $rafaksi->no_raf }}</td>
                        
                        <!-- Kolom 3: Periode Berakhir -->
                        <td style="color: #dc2626; font-weight: bold;">
                            {{ Carbon::parse($rafaksi->periode_akhir)->format('d/m/Y') }}
                        </td>
                        
                        <!-- Kolom 4: Aksi -->
                        <td>
                            <a href="{{ route('rafaksi.renew.index', ['id' => $rafaksi->id]) }}" style="color: #4f46e5; text-decoration: underline; font-weight: bold;">
                                Lihat Detail
                            </a>
                        </td>
                    </tr>
