...
<body>
    <div class="container">
        ...
        <!-- CONTENT -->
        <div class="content">
            <h1 style="color: #dc2626;">URGENT: LOC TELAH KADALUARSA</h1>
            <p>Halo {{ $user->name }},</p>
            <p>Sistem mendeteksi bahwa LOC berikut dari telah <strong style="color: #dc2626;">KADALUARSA (MELEWATI BATAS WAKTU)</strong> namun belum diperbarui di sistem:</p>
            <table>
                <thead>
                    <tr>
                        <th>No</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $loc->no_raf ?? '-' }}</td>
                        <td>{{ $loc->periode_akhir ? \Carbon\Carbon::parse($loc->periode_akhir)->format('d M Y') : '-' }}</td>
                        <td>{{ $loc->tokos->pluck('nama_toko')->implode(', ') ?: '-' }}</td>                        <td>
                        <td>
                            <a href="{{ route('loc.renew.index', ['id' => $loc->id]) }}">
                                Lihat Detail
                            </a>
                        </td>
                    </tr>
