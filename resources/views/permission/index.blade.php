...
@section('content')
    ...
    <div class="mb-5 flex items-center justify-between gap-3">
        ...
        <div id="filterForm" class="mb-6 hidden rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('permission.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <!-- Nama Permission -->
                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Nama Permission</label>
                    <input type="text" id="name" name="name" value="{{ request('name') }}"
                        placeholder="Cari nama permission..." class="w-full rounded-md border-gray-300 border px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('permission.index') }}" class="flex-1 border border-gray-300 bg-white px-4 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-100 text-center">Reset</a>
                </div>
            </form>
        </div>
        ...
        <table id="permissionTable" class="uppercase min-w-full table-fixed divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr class="bg-gray-50 text-left text-xs uppercase tracking-wider text-gray-600">
                    <th class="w-16 px-4 py-3 !font-black">No</th>
                    <th class="w-56 px-4 py-3 !font-black">Nama Permission</th>
                    {{-- <th class="w-96 px-4 py-3 !font-black">Description</th> --}}
            ...
            <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap items-center gap-2">
                                <a class="inline-flex items-center rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-xs font-medium hover:bg-gray-100" href="{{ route('permission.edit', $item->id) }}">Edit</a>
                                <form action="{{ route('permission.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-4 text-center text-gray-500">Belum ada data permission.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
