...
<div class="mx-auto max-w-7xl px-4 py-8">
    ...
    <div class="flex ms-auto m-4 gap-2">
        ...
        <a href="{{ route('loc.print', array_merge(['year' => $year, 'month' => $month], request()->only(['category_id', 'toko_id']))) }}" class="bg-red-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700 transition">
            Print
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table id="locDetail" class="w-full text-left text-sm text-gray-600">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($locs as $loc)
                        <tr class="hover:bg-gray-50 transition-colors" data-nominal="{{ $loc->nominal }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-semibold text-gray-800">{{ $loc->supplier_code }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-xs font-semibold text-gray-700">
                                    {{ \Carbon\Carbon::parse($loc->periode_awal)->format('d M Y') }} 
                                    <span class="text-gray-400 mx-1">-</span> 
                                    {{ \Carbon\Carbon::parse($loc->periode_akhir)->format('d M Y') }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="bg-gray-100 text-gray-700 border border-gray-200 font-semibold px-2 py-1 rounded text-xs">
                                    {{ optional($loc->tokos->first())->nama_toko ?? '-' }}
                                </span>
                            </td>
