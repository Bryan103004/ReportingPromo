...
<div class="mx-auto max-w-7xl px-4 py-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="bg-blue-600 px-6 py-4 flex justify-between items-center">
            <h5 class="text-lg font-bold text-white m-0">Edit Data Loc</h5>
            <a href="{{ route('loc.index') }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm">
                Kembali
            </a>
        </div>
        ...
        <form action="{{route('loc.update', $loc->id)}}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="relative flex-1">
                            <select name="supplier_code" id="choices-supplier" 
                                ...
                                @foreach ($supplierRafaksi as $supplier)
                                    <option value="{{ $supplier->kode_supplier }}" {{ old('supplier_code', $loc->supplier_code) == $supplier->kode_supplier ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="supplier_name" id="supplier_name" value="{{ old('supplier_name', $loc->supplier_name) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Otomatis Terisi Saat Anda Memilih Kode Supplier" readonly required>
                </div>    
                    ...
                    <div class="relative">
                        <select name="category_id" id="category_id" 
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $loc->category_id) == $category->id ? 'selected' : '' }} class="text-gray-900">
                ...
                <div>
                    ...
                    <input type="text" name="no_raf" id="no_raf" value="{{ old('no_raf', $loc->no_raf) }}" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="Contoh: RAF/2026/001" required>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Status Email<span class="font-bold text-red-600">*</span></label>
                    <select name="status_email" class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" required>
                        <option value="aktif" {{ old('status_email', $loc->status_email) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="tidak_aktif" {{ old('status_email', $loc->status_email) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    </select>
                </div>
            ...
            <div class="mb-8 mx-6">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="nominal" id="nominal" min="0" step="any" value="{{ old('nominal', $loc->nominal) }}" class="w-full rounded-md border border-gray-300 pl-10 pr-4 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-colors" placeholder="0" required>
                </div>
            </div> 
