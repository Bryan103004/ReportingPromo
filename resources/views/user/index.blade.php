...
<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div class="mb-5 flex users-center justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Daftar user yang terdaftar.</p>
        </div>
    ...
    <div class="mb-6">
        <form method="GET" action="{{ route('user.index') }}" class="flex gap-2">
            <div class="flex-1 relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama user, username, atau email..." class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 pl-10">
                {{-- <svg class="h-5 w-5 absolute left-3 top-2.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg> --}}
            </div>
            <button type="submit" class="rounded-md border border-blue-300 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 hover:bg-blue-100 transition-colors">Cari</button>
            <a href="{{ route('user.index') }}" class="rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors">Reset</a>
        </form>
    </div>
    ...
    {{-- Form Paginate Atas --}}
    <form method="GET" action="{{ route('user.index') }}" id="paginateForm" class="inline-block mb-4">
    ...
    <table class="uppercase min-w-full table-fixed divide-y divide-gray-200 text-sm">
        <thead>
    ...
    <form method="GET" action="{{ route('user.index') }}" id="paginateFormBottom" class="inline-block mt-2 mb-2">
        @foreach(request()->except('number') as $key => $val)
            @if(is_array($val))
                @foreach($val as $v)
