...
@section('content')
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div class="max-w-xl mx-auto p-4 bg-white rounded-xl shadow-sm border border-gray-100">
            <h3 class="text-sm font-semibold text-gray-700 mb-3">Hak Akses Supervisor</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <!-- Supervisor Rafaksi -->
                <label class="relative flex items-center p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition-all select-none has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50/50">
                    <!-- Hidden input sebagai fallback nilai 0 -->
                    <input type="hidden" name="is_rafaksi_spv" value="0">
                    <input type="checkbox" name="is_rafaksi_spv" value="1" {{ old('is_rafaksi_spv', $user->is_rafaksi_spv) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-3 text-sm font-medium text-gray-700">Supervisor Rafaksi</span>
                </label>

                <!-- Supervisor JSM -->
                <label class="relative flex items-center p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition-all select-none has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50/50">
                    <!-- Hidden input sebagai fallback nilai 0 -->
                    <input type="hidden" name="is_jsm_spv" value="0">
                    <input type="checkbox" name="is_jsm_spv" value="1" {{ old('is_jsm_spv', $user->is_jsm_spv) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-3 text-sm font-medium text-gray-700">Supervisor JSM</span>
                </label>

                <!-- Supervisor PWP -->
                <label class="relative flex items-center p-3 rounded-lg border border-gray-200 cursor-pointer hover:bg-gray-50 transition-all select-none has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50/50">
                    <!-- Hidden input sebagai fallback nilai 0 -->
                    <input type="hidden" name="is_pwp_spv" value="0">
                    <input type="checkbox" name="is_pwp_spv" value="1" {{ old('is_pwp_spv', $user->is_pwp_spv) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-3 text-sm font-medium text-gray-700">Supervisor PWP</span>
                </label>
            </div>
        </div>
        ...
        <div>
            ...
            @if($user->signature_path)
                ...
                <div class="mt-2">
                    <img class="max-w-xs" src="data:image/png;base64,{{ base64_encode(Storage::disk('local')->get($user->signature_path)) }}" alt="Tanda Tangan" />
                </div>
            @endif
