...
<div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 uppercase tracking-tight">Tambah Penerima</h1>
            <p class="mt-1 text-sm text-gray-500 italic">Daftarkan personel baru untuk menerima email reminder.</p>
        </div>
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Cth: Bryan" required class="w-full rounded-xl border-gray-200 bg-slate-50 text-sm font-bold focus:ring-indigo-500 shadow-inner p-3">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Cth: bryan@grandlucky.co.id" required class="w-full rounded-xl border-gray-200 bg-slate-50 text-sm font-mono focus:ring-indigo-500 shadow-inner p-3">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        @foreach($modules as $key => $label)
                            <label class="relative flex cursor-pointer rounded-xl border border-gray-200 bg-white p-4 shadow-sm focus:outline-none hover:bg-slate-50 transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50/30 group">
                                <input type="checkbox" name="modules[]" value="{{ $key }}" class="module-checkbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ is_array(old('modules')) && in_array($key, old('modules')) ? 'checked' : '' }}>
                                <span class="ml-3">
                    ...
                    @error('modules') <p class="mt-2 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
