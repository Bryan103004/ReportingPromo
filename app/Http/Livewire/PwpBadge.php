...
class PwpBadge extends Component
{
    ...
    public function render()
    {
        ...
        $data = DB::table('pwps as p')
            ->select([
                DB::raw("COUNT(DISTINCT CASE WHEN p.periode_akhir > '{$today}' THEN p.id END) as `aktif`"),
                DB::raw("COUNT(DISTINCT CASE WHEN p.periode_akhir <= '{$today}' THEN p.id END) as `expired`")
            ])
